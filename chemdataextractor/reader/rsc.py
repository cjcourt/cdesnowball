...
from ..scrape.pub.rsc import replace_rsc_img_chars, space_references
...
class RscHtmlReader(HtmlReader):
    ...
    table_css = 'div[class="rtable__wrapper"], table'
    ...
    ignore_css = '.table_caption + table, .left_head, sup span.sup_ref, small sup a, a[href^="#fn"], .PMedLink, p[class="header_text"]'
    ...
    def detect(self, fstring, fname=None):
        ...
        if fname and not (fname.endswith('.html') or fname.endswith('.htm')):
            return False
        if b'content="10.1039/' in fstring:
            return True
        return False
