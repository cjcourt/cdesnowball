...
from ..model import Compound, UvvisSpectrum, UvvisPeak, QuantumYield, FluorescenceLifetime, MeltingPoint, GlassTransition, NeelTemperature, CurieTemperature
...
subject_phrase = ((I('of') | I('for') | I('in')) + chemical_name)('subject_phrase')
...
temp_units = ((W('°') | W('∘')) + R('[CFK]') | W('K'))('units').add_action(merge)
...
# Neel temperature table parser  - CC889
neel_temp_title = (R('^T(Néel|néel|n|N|neel|Neel)$') | (W('T') + R('^(Néel|néel|n|N|neel|Neel)')) | ((W('Néel') | W('Neel')) + (R('^temp[\.]?$') | I('temperature'))) | I('Property'))
theory_or_calc = R('^(exp|experimental|cal|calculated|theory|theoretical|predicted)$')
neel_temp_heading = (neel_temp_title + Optional(theory_or_calc).hide() + Optional(delims).hide()  + Optional(lbrct).hide() + Optional(temp_units) + Optional(rbrct).hide())('neel_temp_heading')
neel_temp_cell = Optional(neel_temp_title) + Not(I('TC')) + Optional(I('=')) + (temp_with_optional_units + ZeroOrMore(delims.hide() + temp_with_optional_units))('neel_temp_cell')
doping_level_title = (R('^[xyz]$'))('doping_level_heading')

# TODO: Get doping levels from tables
doping_level_cell = (Optional(R('^[xyz]$')) + Optional(W('=')) + R('^(\d.?)+$'))('doping_level_cell')

# curie temperature table parser - CC889
curie_temp_title = (R('^T(Curie|curie|C|c)$') | (W('T') + R('^(Curie|curie|C|c)')) | ((W('Curie')) + (R('^temp[\.]?$') | I('temperature'))) | I('Property'))
theory_or_calc = R('^(exp|experimental|cal|calculated|theory|theoretical|predicted)$')
curie_temp_heading = (curie_temp_title.hide() + Optional(theory_or_calc).hide() + Optional(delims).hide()  + Optional(lbrct).hide() + Optional(temp_units) + Optional(rbrct).hide())('neel_temp_heading')
curie_temp_cell = Optional(curie_temp_title) + Optional(I('=')) + Not(I('TN')) + (temp_with_optional_units + ZeroOrMore(delims.hide() + temp_with_optional_units))('neel_temp_cell')
...
class NeelTemperatureHeadingParser(BaseParser):
    """"""
    root = neel_temp_heading

    def interpret(self, result, start, end):
        """"""
        neel_temp_units = first(result.xpath('./units/text()'))
        c = Compound()
        if neel_temp_units:
            c.neel_temperatures.append(
                NeelTemperature(units=neel_temp_units)
            )
        yield c


class NeelTemperatureCellParser(BaseParser):
    """"""
    root = neel_temp_cell
    ...
    def interpret(self, result, start, end):
        """"""
        c = Compound()
        for nt in result.xpath('./temp'):
            #if nt.xpath('./temp'):
            c.neel_temperatures.append(
                 NeelTemperature(
                    value=first(nt.xpath('./value/text()')),
...
class CurieTemperatureHeadingParser(BaseParser):
    """"""
    root = curie_temp_heading

    def interpret(self, result, start, end):
        """"""
        curie_temp_units = first(result.xpath('./units/text()'))
        c = Compound()
        if curie_temp_units:
            c.curie_temperatures.append(
                CurieTemperature(units=curie_temp_units)
            )
        yield c


class CurieTemperatureCellParser(BaseParser):
    """"""
    root = curie_temp_cell

    def interpret(self, result, start, end):
        """"""
        c = Compound()
        for ct in result.xpath('./temp'):
            c.curie_temperatures.append(
                CurieTemperature(
                    value=first(ct.xpath('./value/text()')),
...
class CaptionContextParser(BaseParser):
    ...
    def interpret(self, result, start, end):
        ...
        if context:
            c.neel_temperatures = [NeelTemperature(**context)]
        temp = first(result.xpath('./temp_phrase'))
        if temp is not None:
            context['temperature'] = first(temp.xpath('./temp/value/text()'))
            context['temperature_units'] = first(temp.xpath('./temp/units/text()'))
        if context:
            c.curie_temperatures = [CurieTemperature(**context)]
        temp = first(result.xpath('./temp_phrase'))
        if temp is not None:
            context['temperature'] = first(temp.xpath('./temp/value/text()'))
            context['temperature_units'] = first(temp.xpath('./temp/units/text()'))
