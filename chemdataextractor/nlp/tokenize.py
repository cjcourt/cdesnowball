    ...
    def _subspan(self, s, span, nextspan):
        ...
        for i, char in enumerate(text):
            ...
            if char in {':', ';'}:
                ...
                # Split around / unless '+/-' or '-/-' or surrounded by numerics etc.
                if not (before and after and before[-1] in self.NO_SPLIT_SLASH and after[0] in self.NO_SPLIT_SLASH) and not (after and before and after[0].isdigit() and before[-1].isdigit()):
                    return self._split_span(span, i, 1)
            elif char == '>':
